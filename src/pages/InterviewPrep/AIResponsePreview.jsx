                    ...
                    components={{
                        code({ node, className, children, ...props }) {
                            ...
                            return !isInline ? (
                                ...
                                <code  className='px-1 py-0.5 bg-[#0e0e0e] text-[#8ba2eb] rounded text-sm' {...props}>
                                    {children}
                                </code>
                            )
                        },
                        ...
                        ol({ children }) {
                            return <ol className="list-discimal pl-6 space-y-2 my-4">{children}</ol>;
                        },
                        li({ children }) {
                        ...
                        table({ children }) {
                            return (
                                <div className="overflow-x-auto my-4">
                                    <table className="min-2-w-full divide-y divide-gray-300 border border-gray-200">
                                        {children}
                                    </table>
                                </div>
                            );
                        ...
                        thead({ children }) {
                            return <thead className="bg-[#282828]">{children}</thead>;
                        },
                        tbody({ children }) {
                        ...
                        th({ children }) {
                            return <th className="px-3 py-2 text-left text-xs font-medium text-[#ddf0fe]">{children}</th>;
                        },
                        td({ children }) {
...
function CodeBlock({ code, language }) {
    ...
    return <div className="relative my-6 rounded-lg overflow-hidden bg-[#282828] border-gray-200">
        <div className="flex items-center justify-between px-4 py-2 bg-[#0e0e0e] border-gray-200">
            <div className="flex items-center space-x-2">
                <LuCode size={16} className="text-[#ddf0fe]" />
            ...
            <button
                onClick={copyCode}
                className="text-[#ddf0fe] ☐ hover:text-gray-700 focus:outline-none relative group"
                aria-label="Copy_code"
            >
                {copied ? (
